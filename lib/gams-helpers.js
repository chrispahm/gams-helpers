...
function ensureDirectoryExistence(filePath) {
	var dirname = path.normalize(filePath)
...
function createCSON(dir, flag) {
	dir = dir.replace(/\\/g, '\\\\\\\\')
	const gdxPath = path.resolve(__dirname, '..', 'scrdir', 'GAMS-Helper.gdx').replace(/\\/g, '\\\\\\\\')
	const string = `'.source.gams':
	  'Check':
	    'prefix': 'c'
	    'description': 'Generic command to output set/parameter/variable to website'
	    'body': """
				execute_unload "${gdxPath}", $1;
				abort $1;
			"""`
	ensureDirectoryExistence(__dirname + '/../snippets/')
...
export default {
	...
	run(callback) {
		...
		if (editor = atom.workspace.getActiveTextEditor()) {
			...
			findUp(['exp_starter.gms', 'capmod.gms'], directory).then((expStarter) => {
				...
				// run the model, and print the stdout to the console during execution
				this.consolePanel.log(gamsParams)
				const run = shell.exec(gamsParams, (code,stderr,stdout) => {
					this.provider.clear()
					this.consolePanel.log(stdout)
					this.consolePanel.log(stderr)
					this.consolePanel.log(code)
					//					this.consolePanel.toggle()
	...
	check() {
		...
		if (editor = atom.workspace.getActiveTextEditor()) {
			...
			this.run(() => {
				PythonShell.run(atom.config.get('gams-helpers.Scratch directory') + path.sep + '..' + path.sep + 'lib' + path.sep + 'importGDX.py', {args: [gdxFile]}, (err, results) => {
					if (err) throw err
					...
					pythonPath: path.dirname(atom.config.get('gams-helpers.Scratch directory')) + path.sep + 'GMSPython'
				})
			})
		}
	}
