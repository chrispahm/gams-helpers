      ...
      findUp(entryFiles, {cwd: directory}).then((entryFile) => {
        if (entryFile && path.parse(entryFile).base !== '.gamslintc.js') {
          ...
          if (gamsFile === 'exp_starter.gms') {
            parameters = `-scrdir="${scratchdir}" --scen=incgen/runInc`
          ...
          parameters += ` ${atom.config.get('linter-gams.Command Line Arguments - Execution').join(' ')} `
        ...
        const gamsParams = gamsExe + ' "' + filePath + '" PS=0  -scrdir="' + scratchdir + '" ' + ' --scrdir="' + scratchdir + '" ' + ' -workdir="' + directory + '" ' + ' -curDir="' + directory + '" ' + parameters + extraParams
