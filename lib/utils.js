	...
	execLine(filePath,directory,extraParams) {
		return new Promise((resolve) => {
			...
			findUp(['exp_starter.gms', 'capmod.gms'], directory).then((expStarter) => {
				...
				let dmpPath = path.parse(filePath)
				dmpPath.ext = '.dmp'
				dmpPath.base = ''
				dmpPath = path.format(dmpPath)

				let refPath = path.parse(filePath)
				refPath.ext = '.dmp'
				refPath.base = ''
				refPath = path.format(refPath)
// PS=0 dispWidth=20 a=c on115=0 symbol=test.txt rf=test.ref fErr="test.err suppress=0"
